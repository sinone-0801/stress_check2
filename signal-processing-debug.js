 ...
 * 信号処理関連の機能を提供するモジュール（デバッグ用短時間測定版）
 */
...
class SignalProcessor {
    ...
    addDataPoint(ppgValue, timestamp) {
        ...
        // デバッグ用：短時間でも解析できるようにバッファサイズの閾値を下げる
        if (this.ppgBuffer.length >= 15) { // 最低0.5秒分のデータに縮小
            try {
                // LFとHFに帯域分割
    ...
    /**
     * 瞬時振幅を計算（デバッグ用に窓サイズを縮小）
     */
    calculateInstantaneousAmplitude() {
        // デバッグ用に必要な最小バッファサイズを縮小
        if (this.lfBuffer.length < this.ENVELOPE_WINDOW_SIZE * 1.5 || 
            this.hfBuffer.length < this.ENVELOPE_WINDOW_SIZE * 1.5) {
            return;
        }
    ...
    normalizeAndScaleAmplitudes() {
        ...
        try {
            ...
            // デバッグ用に少ないデータでも処理できるよう閾値を下げる
            if (validLfValues.length > 3) {
                // 外れ値に影響されにくいように10-90パーセンタイルを使用
                validLfValues.sort((a, b) => a - b);
                const lowerIdx = Math.floor(validLfValues.length * 0.1);
                const upperIdx = Math.floor(validLfValues.length * 0.9);
                
                lfMin = validLfValues[lowerIdx];
                lfMax = validLfValues[upperIdx];
            }
            
            if (validHfValues.length > 3) {
                validHfValues.sort((a, b) => a - b);
                const lowerIdx = Math.floor(validHfValues.length * 0.1);
                const upperIdx = Math.floor(validHfValues.length * 0.9);
    ...
    addRriData(rriValue, timestamp) {
        ...
        // バッファサイズを制限（デバッグ用に短くする）
        if (this.rriBuffer.length > 30) {
            this.rriBuffer.shift();
            this.rriTimeBuffer.shift();
        }
        
        // RRIデータからHRVパラメータの再計算（デバッグ用に閾値を下げる）
        if (this.rriBuffer.length >= 4) { // 最低4拍分に縮小
    ...
    calculateHRVFromRRI() {
        try {
            // RRIデータが十分あるか確認
            if (this.rriBuffer.length < 4) return;
